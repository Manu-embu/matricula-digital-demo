const GEO_ENGINE_VERSION='0.5.8';
const views = [...document.querySelectorAll('.view')];
const steps = [...document.querySelectorAll('.step')];
let currentStep = 1;
let submittingInscricao = false;
let cpfDuplicadoDetectado = false;
...
function popularEscolasIrmao(){
  const sel=document.getElementById('irmaoEscola');
  if(!sel || !Array.isArray(unidadesSME)) return;
  const atual=sel.value;
  const nomes=[...new Set(unidadesSME.map(u=>String(u.nome_exibicao||u.nome_oficial||'').trim()).filter(Boolean))]
...
async function verificarCpfInscricaoExistente(){
  const el=document.getElementById('cpfEstudante');
  const box=document.getElementById('cpfEstudanteBox');
  const cpf=onlyDigits(el?.value||'');

  cpfDuplicadoDetectado=false;
  if(!cpf){
  ...
  if(!cpfValido(cpf)){
    if(box){
      box.innerHTML='<strong>CPF inválido.</strong><br>Confira o número informado.';
  ...
  try{
    const resp=await WorkspaceBridge.request({acao:'consultarCpfInscrito',cpf});
    cpfDuplicadoDetectado=!!resp?.existe;
    if(box){
      if(cpfDuplicadoDetectado){
        box.innerHTML='<div class="blocked-result"><strong>Já existe uma inscrição para este CPF.</strong><br>Utilize a opção <strong>Consultar inscrição</strong> para acompanhar o protocolo já registrado.</div>';
        box.classList.remove('hidden');
      }else{
        box.innerHTML='<div class="approved-result"><strong>CPF disponível para nova inscrição.</strong></div>';
        box.classList.remove('hidden');
      }
    }
    return cpfDuplicadoDetectado;
  }catch(err){
    console.warn(err);
    return false;
  }
...
document.getElementById('cpfEstudante').addEventListener('input', e=>{maskCPF(e.target);cpfDuplicadoDetectado=false;document.getElementById('cpfEstudanteBox')?.classList.add('hidden');});
document.getElementById('cpfEstudante').addEventListener('blur', verificarCpfInscricaoExistente);
...
document.getElementById('criterioIrmaoMesmaEscola')?.addEventListener('change',e=>{
  const wrap=document.getElementById('irmaoEscolaWrap');
  const sel=document.getElementById('irmaoEscola');
  if(e.target.checked){
    wrap?.classList.remove('hidden');
...
function calculateStage(){
  ...
  if(d>cutoff){
    box.innerHTML='<strong>Verifique a data</strong><br>A data de nascimento informada não é compatível com o processo de inscrição.';
    box.classList.remove('hidden');
    return;
  }
  ...
  box.innerHTML=`<strong>Resultado preliminar</strong><br>${etapa}<br><small>Etapa estimada a partir da data de nascimento. A confirmação ocorrerá na análise da inscrição.</small>`;
  box.classList.remove('hidden');
  updatePeriodOptions();
}
...
function showGeoSuccess(box, precisionText, roadLabel){
  ...
  box.innerHTML=`<strong>${approx?'Endereço confirmado — referência territorial':'Endereço localizado e conferido'}</strong><br>${geocodedAddress}<br>
    <small>${precisionText}${roadLabel?` • ${roadLabel}`:''}</small><br>
    <div class="warning-box"><strong>Importante:</strong> a localização será utilizada pelo Setor de Demanda para análise da unidade mais adequada, considerando proximidade e disponibilidade de vaga.</div>`;
  box.classList.remove('hidden');
}
...
function validateStep(){
  ...
  if(currentStep===2){
    ...
    const cpfInput=document.getElementById('cpfEstudante');
    ...
    if(cpfDuplicadoDetectado){
      cpfInput.focus();
      alert('Já existe uma inscrição para este CPF.');
      return false;
    }
  }
...
function buildReview(){
  const criterios=criteriosDeclarados();
  const labels={
    PCD:'Criança com deficiência / necessidade especial',
    DOENCA_RARA:'Doença rara / condição de saúde prioritária',
    FAMILIA_MONOPARENTAL:'Família monoparental',
    MAE_ADOLESCENTE:'Mãe adolescente',
    IRMAO_MESMA_ESCOLA:'Irmão(ã) em unidade municipal',
    BOLSA_FAMILIA:'Beneficiário Bolsa Família',
    VULNERABILIDADE_PROTECAO:'Vulnerabilidade social / proteção'
  };
  const criteriosTxt=criterios.length
    ? criterios.map(c=>labels[c]||c).join(', ')
  ...
  document.getElementById('review').innerHTML=`
    ...
    <div><small>CPF</small><strong>${value('cpfEstudante')}</strong></div>
    ...
    <div><small>Critérios declarados</small><strong>${criteriosTxt}</strong></div>
...
const CRITERIOS_PRIORIDADE=[
 ['criterioPcd','PCD'],
 ['criterioDoencaRara','DOENCA_RARA'],
 ['criterioFamiliaMonoparental','FAMILIA_MONOPARENTAL'],
 ['criterioMaeAdolescente','MAE_ADOLESCENTE'],
 ['criterioIrmaoMesmaEscola','IRMAO_MESMA_ESCOLA'],
 ['criterioBolsaFamilia','BOLSA_FAMILIA'],
 ['criterioVulnerabilidade','VULNERABILIDADE_PROTECAO']
];
function criteriosDeclarados(){
  return CRITERIOS_PRIORIDADE
    .filter(([id])=>document.getElementById(id)?.checked)
    .map(([,c])=>c);
}

function finish(){
  if(submittingInscricao) return;
  ...
  setHidden(form,'unidadeIndicada','');
  ...
  setHidden(form,'irmaoEscolaDeclarada',document.getElementById('irmaoEscola')?.value||'');
  setHidden(form,'classificacaoPeriodo',registrationWindowCode());
  setHidden(form,'dataCliente',todayLocalISO());

  submittingInscricao=true;
  const nextBtn=document.getElementById('btnNext');
  const prevBtn=document.getElementById('btnPrev');
  const indicator=document.getElementById('saveIndicator');
  const restore=showBusy(nextBtn,'Salvando inscrição...');
  ...
  WorkspaceBridge.submitForm(form)
    .then(resp=>{
      if(!resp?.ok){
        throw new Error(resp?.message || 'Não foi possível concluir a inscrição.');
      }
...
function verifyActiveEnrollment(){
  ...
  if(!dataValidaBR(nascimento)){
    box.innerHTML='<strong>Informe uma data de nascimento válida no formato dd/mm/aaaa.</strong>';
    box.classList.remove('hidden'); return;
  }
...
(function configurarPainelDemanda(){
  const btn=document.getElementById('btnPainelDemanda');
  if(!btn) return;
  btn.addEventListener('click',()=>{
    const url=String(window.PAINEL_DEMANDA_URL||'').trim();
    if(!url){
      alert('Painel da Demanda ainda não foi configurado.');
      return;
    }
