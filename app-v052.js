const GEO_ENGINE_VERSION='0.5.2';
...
function renderCompatibleSchools(){
  ...
  if(!residencePoint){
    if(geoStatus==='unverified'){
      box.innerHTML=`<div class="empty-state"><strong>Localização territorial pendente.</strong><br>O CEP/endereço cadastral foi informado, mas não foi possível obter uma coordenada confiável. <strong>Nenhuma unidade será sugerida automaticamente.</strong><br><br>A inscrição poderá continuar e ficará marcada para <strong>análise territorial pela SME</strong>.</div>`;
    }else{
  ...
  const principais=list.slice(0,LIMITE_INICIAL);
  const adicionais=list.slice(LIMITE_INICIAL);

  function badgeRank(idx){
    if(idx===0) return '<span class="nearest-badge">Mais próxima</span>';
    if(idx===1) return '<span class="nearest-badge rank-secondary">2ª mais próxima</span>';
    if(idx===2) return '<span class="nearest-badge rank-secondary">3ª mais próxima</span>';
    return '';
  }
  ...
  Exibindo primeiro as <strong>${Math.min(LIMITE_INICIAL,list.length)} mais próximas</strong>, em ordem de distância.
  <br><small>Protótipo: distância geográfica em linha reta. Produção: rota a pé.</small></div>`;

  const principaisHtml=principais.map((item,idx)=>schoolCard(item,idx)).join('');
  const adicionaisHtml=adicionais.length ? `
    <button type="button" class="more-units-btn" id="toggleMoreUnits">
      + Ver outras ${adicionais.length} unidade(s) dentro de 2 km
    </button>
    <div id="moreUnitsList" class="more-units-list" hidden>
      ${adicionais.map((item,idx)=>schoolCard(item,idx+LIMITE_INICIAL)).join('')}
    </div>` : '';

  box.innerHTML=intro+principaisHtml+adicionaisHtml;

  const toggle=document.getElementById('toggleMoreUnits');
  const more=document.getElementById('moreUnitsList');
  if(toggle && more){
    toggle.addEventListener('click',()=>{
      const opening=more.hidden;
      more.hidden=!opening;
      toggle.textContent=opening
        ? `− Ocultar outras ${adicionais.length} unidade(s)`
        : `+ Ver outras ${adicionais.length} unidade(s) dentro de 2 km`;
    });
  }
}
...
function showGeoSuccess(box, precisionText, roadLabel){
  ...
  const inside=geocoded.filter(u=>haversineKm(residencePoint,{lat:Number(u.latitude),lon:Number(u.longitude)})<=2);
  const approx=geoStatus==='approximate';
  box.innerHTML=`<strong>${approx?'Endereço confirmado — localização aproximada':'Endereço localizado e conferido'}</strong><br>${geocodedAddress}<br>
    ...
    <div class=\"warning-box\"><strong>${approx?'Atenção:':'Importante:'}</strong> ${approx?(geoSource.includes('SME')?'a coordenada é uma referência territorial validada para o CEP/logradouro e ainda não representa o ponto exato do imóvel. ':'a coordenada é aproximada e serve apenas para triagem inicial. '):''}Nesta demonstração, a distância ainda é em linha reta. Na produção, a regra será calculada por <strong>rota a pé</strong>.</div>`;
  box.classList.remove('hidden');
}
...
function hasDeclaredPriority(){
  return !!(document.getElementById('pcd')?.checked || document.getElementById('doencaRara')?.checked);
}
...
function finish(){
  ...
  setHidden(form,'tiposPrioridade',priorityTypes().join('; '));
...
function resetPrecheck(){
  ['checkNome','checkNascimento','checkDocumento'].forEach(id=>{const el=document.getElementById(id); if(el)el.value='';});
  document.getElementById('precheckResult')?.classList.add('hidden');
}
function normalizeDoc(v){return onlyDigits(v||'');}
...
function verifyActiveEnrollment(){
  ...
  const documento=document.getElementById('checkDocumento').value.trim();
  const box=document.getElementById('precheckResult');

  if(!nome || !nascimento || !documento){
    box.innerHTML='<strong>Preencha os três campos.</strong><br>Informe nome, data de nascimento e RA/CPF para realizar a verificação.';
    box.classList.remove('hidden'); return;
  }
  ...
  WorkspaceBridge.request({
    acao:'consultarMatriculaAtiva',
    nome,nascimento,documento
  }).then(resp=>{
    restore();
    if(resp?.matriculado){
      box.innerHTML=`<div class="blocked-result"><strong>Já existe matrícula ativa.</strong><br>
        ${resp.nome||nome} consta com matrícula ativa${resp.escola?` em <strong>${resp.escola}</strong>`:''}.<br><br>
        <strong>Não será aberta nova inscrição.</strong> Este sistema é destinado a quem está fora da escola.
        Para transferência ou assuntos da matrícula atual, procure diretamente a unidade escolar.</div>`;
      box.classList.remove('hidden'); return;
    }

    box.innerHTML=`<div class="approved-result"><strong>Nenhuma matrícula ativa localizada.</strong><br>
      O formulário de inscrição pode ser iniciado.</div>
      <button type="button" class="primary precheck-continue" id="btnIniciarFormulario">Iniciar inscrição</button>`;
    box.classList.remove('hidden');
    ...
    document.getElementById('btnIniciarFormulario').addEventListener('click',()=>{
      ...
      if(normalizeDoc(documento).length===11) document.getElementById('cpfEstudante').value=documento;
      calculateStage();
      currentStep=1; updateProgress(); showView('wizard');
    });
  }).catch(err=>{
    restore();
    alert('Erro ao consultar matrícula: '+(err?.message||err));
  });
}
