const GEO_ENGINE_VERSION='0.5.3a';
...
function priorizarPorTerritorio(list){
  ...
  if(!chave) return {lista:ordenarDist(list).slice(0,3), territorial:false};
  ...
  if(doTerritorio.length>=3){
    return {lista:doTerritorio.slice(0,3), territorial:true, territorio:chave};
  }

  const usados=new Set(doTerritorio.map(x=>x.u.nome_exibicao));
  const complemento=ordenarDist(list.filter(x=>!usados.has(x.u.nome_exibicao)));
