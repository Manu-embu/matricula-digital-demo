const GEO_ENGINE_VERSION='0.4.6';
...
document.getElementById('btnSimularEndereco').addEventListener('click', async ()=>{
  ...
  try{
    ...
    const scored=uniq.map(r=>{
      ...
      const cityOk=cityText.includes('embu das artes');
      const roadScore=tokenSimilarity(wantedRoad,road);
      const bairroScore=tokenSimilarity(wantedBairro,[a.suburb,a.neighbourhood,a.quarter,a.city_district].filter(Boolean).join(' '));
      const post=onlyDigits(a.postcode||'');
      const cepExact=!!cep && !!post && post===cep;
      const cepConflict=!!cep && !!post && post!==cep;
      const score=(cityOk?4:0)+(roadScore*8)+(bairroScore*2)+(cepExact?3:0)-(cepConflict?8:0);
      return {r,cityOk,roadScore,bairroScore,cepExact,cepConflict,score,road,post};
    }).filter(x=>x.cityOk && x.roadScore>=0.80 && !x.cepConflict)
      .sort((a,b)=>b.score-a.score);

    const best=scored.find(x=>x.road && (x.bairroScore>=0.34 || x.cepExact || !cep));
    if(!best){
      residencePoint=null;
      geocodedAddress='';
      ...
      box.innerHTML=`<strong>Não foi possível confirmar este endereço com segurança.</strong><br>O sistema <strong>não indicará nenhuma escola</strong> enquanto a rua/localização não for validada.${cepNote}<br><br>Revise o endereço ou prossiga para análise territorial pela SME.<br><small>Motor geográfico v${GEO_ENGINE_VERSION}</small>`;
      box.classList.remove('hidden');
      return;
    }
    ...
    box.innerHTML=`<strong>Endereço localizado e conferido</strong><br>${geocodedAddress}<br><small>Motor geográfico v${GEO_ENGINE_VERSION} • logradouro validado: ${best.road}</small><br>
