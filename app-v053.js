const GEO_ENGINE_VERSION='0.5.3';
...
function priorizarPorTerritorio(list){
  ...
  if(!chave) return {lista:[...list].sort((a,b)=>a.dist-b.dist).slice(0,3), territorial:false};

  const ordem = TERRITORIOS_ESCOLARES_PILOTO[chave]
    .map(normalizarTerritorioTexto);
  ...
  if(doTerritorio.length>=3) return {lista:doTerritorio.slice(0,3), territorial:true, territorio:chave};

  // Completa com as mais próximas somente se a lista territorial ainda não tiver 3 unidades compatíveis.
  const usados=new Set(doTerritorio.map(x=>x.u.nome_exibicao));
  const complemento=[...list].filter(x=>!usados.has(x.u.nome_exibicao)).sort((a,b)=>a.dist-b.dist);
  return {lista:[...doTerritorio,...complemento].slice(0,3), territorial:true, territorio:chave};
}
...
function renderCompatibleSchools(){
  ...
  // Mantemos um universo técnico amplo para localizar candidatas, mas não exibimos a lista inteira à família.
  const universo=allOffers
    .filter(x=>x.hasCoords && x.dist!==null && x.dist<=3)
    .sort((a,b)=>a.dist-b.dist);

  if(!universo.length){
    box.innerHTML=`<div class="empty-state"><strong>Nenhuma unidade compatível foi localizada para triagem automática.</strong><br>A inscrição poderá continuar e será encaminhada para análise territorial pela SME.</div>`;
    return;
  }
  ...
  function schoolCard(item,idx){
    ...
    return `<label class="school-option">
      ...
      <span>
        ...
        <small>${u.bairro?u.bairro+' • ':''}${dist.toFixed(2).replace('.',',')} km <em>(referência geográfica)</em></small>
