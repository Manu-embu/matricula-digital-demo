const GEO_ENGINE_VERSION='0.4.7';
...
async function autofillCep(){
  ...
  if(city && city!=='embu das artes'){
    residencePoint=null; geocodedAddress=''; geoStatus='unverified';
...
function showGeoSuccess(box, precisionText, roadLabel){
  ...
  box.innerHTML=`<strong>${approx?'Endereço confirmado — localização aproximada':'Endereço localizado e conferido'}</strong><br>${geocodedAddress}<br>
    <small>Motor geográfico v${GEO_ENGINE_VERSION} • ${precisionText}${roadLabel?` • ${roadLabel}`:''}</small><br>
    ...
    <div class=\"warning-box\"><strong>${approx?'Atenção:':'Importante:'}</strong> ${approx?'a coordenada é aproximada e serve apenas para triagem inicial. ':''}Nesta demonstração, a distância ainda é em linha reta. Na produção, a regra será calculada por <strong>rota a pé</strong>.</div>`;
  box.classList.remove('hidden');
}
